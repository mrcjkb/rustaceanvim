---@mod ferris.health Health checks
...
local function check_config()
  ...
  if vim.g.ferris then
    ok('vim.g.ferris is set')
  else
    ok('vim.g.ferris is not set')
  end
  local valid, err = require('ferris.config.check').validate(config)
  if valid then
    ok('No errors found in config.')
  else
    error(err or '' .. vim.g.ferris and '' or ' This looks like a plugin bug!')
  end
end
...
local function check_for_conflicts()
  ...
  for _, autocmd in ipairs(vim.api.nvim_get_autocmds { event = 'FileType', pattern = 'rust' }) do
    if autocmd.group_name and autocmd.group_name == 'lspconfig' then
      error('lspconfig.rust_analyzer has been setup. This will likely lead to conflicts with the ferris LSP client.')
      return
    end
  end
